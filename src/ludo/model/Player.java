...
public class Player {
    ...
    /** Danh sách ngựa có thể di chuyển với 2 xúc xắc */
        ...
        for (Horse h : horses) {
            ...
            if (h.getState() == HorseState.IN_BASE) {
                ...
                if (dist < 55) {
                    ...
                    // Ở cửa chuồng, cần đổ được 1 (v1, v2, hoặc sum == 1)
    ...
    public boolean hasHorseInBase() {
        for (Horse h : horses) {
            if (h.getState() == HorseState.IN_BASE) return true;
        }
        return false;
    }

    public Horse getFirstHorseInBase() {
        for (Horse h : horses) {
            if (h.getState() == HorseState.IN_BASE) return h;
        }
        return null;
    }
